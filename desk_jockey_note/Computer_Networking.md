...
除了复用、分用功能，还有一些轻量级的错误检查，它再也没有往ip上增加什么东西了。如果应用开发者选择UDP而不是TCP，这个应用几乎是直接和IP对话，UDP直接从应用进程中拿到数据，然后附加上源和目的地端口号字段来为实现复用分用服务，还有2个小字段，然后把这个结果字段传给网络层。网络层把运输层segments注入到IP数据报里，然后把这个部分发到接收端主机。如果segment到达接收端主机，UDP就会使用目的端口号来把segment的数据发到相应的用户进程里。
