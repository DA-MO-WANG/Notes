...
右边的状态，发送端协议等待从接收端拿到ACK或者NAK。如果一个协议中的发送端收到了ack packet，那么就意味着最新的一次传输被正确收到，因为这个发送端回到等待上层数据的状态。如果收到的是NAK，那么发送端就开始重发，直到收到ack。这就意味着，在发送端处于 wait-for-ack-or-nak时，是不可能去接收来自上层的ju j
