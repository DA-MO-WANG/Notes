...
要解释删除表数据却不能回收表空间，先得解释表数据和表空间的关系：

* 关于表数据存储的位置是可以配置的-innodb_file_per_table参数

  * 值为off, 表数据放在系统共享表空间，这个时候表删除了，空间也不会回收

  * 值为on, 表数据以后缀为.ibd的文件存储

* 记录的删除与数据页的删除

  * 记录的删除，就是把这个记录标记为删除，然后位置可以被复用（有相对位置限制）
  * 数据页的删除，数据页本身也可以被复用，任何需要新页的时候都可以被复用（没有相对位置限制）
