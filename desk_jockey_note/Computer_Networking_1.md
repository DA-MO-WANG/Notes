###国立清华-黄能富老师-3rd edition笔记

###### 4.10-1我有印象的部分：

* 直观来看，网络边上是edge(host)，里面是router，两者之间靠link连接
* 从服务角度看，两种，一种无连接、一种面向连接
* 时分复用、频分复用、按需使用（排队）
* tcp的flow-control 和 congest control的区别：流量控制，是怕接收端炸了，让发送端调慢点--只设计两个主机；拥塞控制，是怕网络炸了，当发生丢包时，相应的tcp连接的发送端会调低速度--反应的是环境的情况

###### 4.11-1-44/37

* 讲这个delay: 类似火车站、物流站、飞机场，运作原理是一样的，
  * process delay ：检查有没有bit error，寻找下符合目的地的进站口port
  * queue : 找到port后，就排队
  * transmission：和packet的长度有关，和带宽有关 ，过站时间
  * 媒介：材质不同，速度不同，耗费的时间也不同

###### 4.14-1

* Internet protocol
  * 研究一个复杂的东西，常用的技巧就是模块化，来方便管理
...
rdt 2.0 发送端有两个状态。最左边的状态，发送端等待着来自上层的要传输的数据。当rdt_send事件发送时，发送端就会创造一个packet包裹着要被发送的数据。并添加了checksum。然后通过udt_send发送这个packet。

右边的状态，发送端协议等待从接收端拿到ACK或者NAK。如果一个协议中的发送端收到了ack packet，那么就意味着最新的一次传输被正确收到，因为这个发送端回到等待上层数据的状态。如果收到的是NAK，那么发送端就开始重发，直到收到ack。这就意味着，在发送端处于 wait-for-ack-or-nak时，是不可能去接收来自上层的数据的，除非离开这个状态。因为这种行为，rdt 2.0 b被叫做 staop-and-wait协议。

协议的接收端一方，只有一个状态，就是接收这个packet，然后返回一个ack / nak，取决于packet是否被篡改。在图3.10中，rdt_rcv(rcvpkt) && corrupt(rcvpkt)就是为了当收到的packet 被发现是错误时产生的。

rdt2.0 可能看起来起作用，但是不幸的是它有一个致命的错误。尤其是我们不能确定ACK/NAK packet是否被篡改。不幸的是，我们轻微的疏忽并不是像他看上去那样无害。最低限度，我们将需要在ACK/NAK packet增加checksum 位串，目的是侦查这样的错误。更困难的地方是协议应该怎么从错误中恢复。困难的地方在，如果一个ACK/NAK被篡改，发送者根本没有办法知道接收端是否收到被传输数据的最后一部分。

对于处理被篡改的ACK/NAK，考虑三种可能性：
...
对这个新问题的一个简单解决方式就是增加一个新字段，通过使用sequence number 来给da ta packet编号。接收者只需要检查 sequence number 来决定收到的packet到底是不是重发的。对于stop-and-wait协议，1 bit的sequence number 就足够了。
