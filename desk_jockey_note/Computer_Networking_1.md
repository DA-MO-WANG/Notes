...
rdt2.0 可能看起来起作用，但是不幸的是它有一个致命的错误。尤其是我们不能确定ACK/NAK packet是否被篡改。不幸的是，我们轻微的疏忽并不是像他看上去那样无害。最低限度，我们将需要在ACK/NAK packet增加checksum 位串，目的是侦查这样的错误。更困难的地方是协议应该怎么从错误中恢复。困难的地方在，如果一个ACK/NAK被篡改，发送者根本没有办法知道接收端是否
